...
export class AskService extends Eventable<{}> {
  ...
  async ask(
    ...
    const refinedSources = sources.map((source, idx) => {
      ...
      const evidenceLocation = this._locateEvidence(
        context?.fulltext || "",
        refinedQuote || source.quote || "",
        queriesForSource[0] || trimmedQuestion
      );
      return {
        ...source,
        quote: refinedQuote || source.quote || "",
        page: evidenceLocation.page,
        context: evidenceLocation.context,
        quoteCandidates,
      };
    });
  ...
  private _locateEvidence(
    fulltext: string,
    quote: string,
    query: string
  ): { page?: number; context?: string } {
    const pageBlocks = this._pageBlocks(fulltext);
    if (pageBlocks.length === 0) {
      return { context: quote || "" };
    }

    const quoteCompact = this._compactForMatch(quote);
    const terms = this._queryTerms(query);
    let best = {
      page: pageBlocks[0].page,
      content: pageBlocks[0].content,
      score: -1,
    };

    for (const block of pageBlocks) {
      const lower = block.content.toLowerCase();
      const compact = this._compactForMatch(block.content);
      let score = 0;
      if (
        quoteCompact.length >= 24 &&
        (compact.includes(quoteCompact) || quoteCompact.includes(compact.slice(0, 120)))
      ) {
        score += 100;
      } else if (quoteCompact.length >= 24) {
        const quoteTerms = quoteCompact.match(/.{1,16}/g) || [];
        score += quoteTerms.filter((term) => compact.includes(term)).length * 3;
      }
      score += terms.reduce(
        (sum, term) => sum + (this._hasTerm(lower, compact, term) ? 1 : 0),
        0
      ...
      if (score > best.score) {
        best = { page: block.page, content: block.content, score };
      }
    }

    return {
      page: best.score > 0 ? best.page : undefined,
      context: this._evidenceContext(best.content, quote || query),
    };
  }

  private _pageBlocks(text: string): Array<{ page: number; content: string }> {
    const matches = [...`${text || ""}`.matchAll(/^\[Page\s+(\d+)\]\s*$/gim)];
    if (matches.length === 0) {
      return [];
    }
  ...
  private _evidenceContext(pageContent: string, needle: string): string {
    const normalized = `${pageContent || ""}`.replace(/\s+/g, " ").trim();
    if (!normalized) {
      return "";
    }

    const compactNeedle = this._compactForMatch(needle);
    const windows = this._textCandidates(normalized);
    let best = windows[0] || normalized.slice(0, 520);
    let bestScore = -1;
    const terms = this._queryTerms(needle);
    for (const candidate of windows) {
      const lower = candidate.toLowerCase();
      const compact = this._compactForMatch(candidate);
      let score = terms.reduce(
        (sum, term) => sum + (this._hasTerm(lower, compact, term) ? 1 : 0),
        0
      );
      if (compactNeedle.length >= 24 && compact.includes(compactNeedle.slice(0, 80))) {
        score += 20;
      }
      if (score > bestScore) {
        bestScore = score;
