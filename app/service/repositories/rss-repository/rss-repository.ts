...
export class RSSRepository {
  ...
  async fetch(feed: Feed, timeout = 10000): Promise<FeedEntity[]> {
    const header = {};
  ...
  parseRSSItems(items: RSSItem[]) {
    ...
    for (const item of items) {
      ...
      if (item["pubDate"]) {
        feedEntityDraft.pubTime = this._formatDate(item["pubDate"]);
      } else if (item["prism:coverDate"]) {
        feedEntityDraft.pubTime = this._formatDate(item["prism:coverDate"]);
      }
  ...
  parseAtomItems(items: AtomItem[]) {
    ...
    for (const item of items) {
      ...
      if (item["published"]) {
        feedEntityDraft.pubTime = this._formatDate(item["published"]);
      }
      feedEntityDrafts.push(feedEntityDraft);
    }
      ...
      if (item.description) {
        ...
        for (const component of descriptionComponents) {
          ...
          if (component.startsWith("<p>Publication date:")) {
            ...
            feedEntityDraft.pubTime = this._formatDate(dateStr);
          }
  ...
  private _formatDate(value: string | Date) {
