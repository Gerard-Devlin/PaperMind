...
const sidebarPanelIndex = computed(() =>
  Math.max(0, ["library", "feed"].indexOf(uiState.sidebarPanel))
);

const onViewContentSwitch = (view: number) => {
  if (view === 0) {
    uiState.sidebarPanel = "library";
    uiState.selectedIndex = [];
    uiState.selectedQuerySentenceIds = ["lib-all"];
    uiState.querySentencesSidebar = [];
    uiState.contentType = "library";
  } else {
    uiState.sidebarPanel = "feed";
    uiState.selectedIndex = [];
...
<template>
  <div
    class="flex-none flex flex-col w-full min-w-0 h-screen justify-between overflow-hidden"
  >
