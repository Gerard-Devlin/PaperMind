...
<template>
  <div
    class="flex font-semibold text-xs rounded-md select-none cursor-pointer"
    style="width: 100%"
    ref="titleContainer"
  >
    <div
      class="flex relative min-w-0 shrink-0"
      v-for="([key, template], index) of fieldTemplates.entries()"
      :style="{ width: `${template.width}%` }"
    >
      <div
        class="absolute left-0 top-0 h-full w-1 cursor-col-resize hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-md transition-colors duration-50 z-10"
        v-if="index > 0"
