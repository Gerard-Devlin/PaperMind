...
const compareDownloadSuccess = ref(false);
...
const exportMode = ref<"BibTex" | "PlainText" | "Ask" | "Compare" | "Graph">("BibTex");
const searchText = ref("");
const graphSearchSubmitTick = ref(0);
...
const isGraphMode = computed(() => exportMode.value === "Graph");
const isAssistantMode = computed(() => isAskMode.value || isCompareMode.value || isGraphMode.value);
...
const resizeQuickpaste = async () => {
  ...
  const useIntrinsicContentHeight = isCompareMode.value || isGraphMode.value;
  const contentHeight = useIntrinsicContentHeight
  ...
  const rootExtra = useIntrinsicContentHeight
    ? 0
...
const applyLaunchMode = async () => {
  ...
  if (exportMode.value === "Ask") {
    ...
    await restoreLastCompare();
    await loadComparePapers();
  } else if (exportMode.value === "Graph") {
    askAnswer.value = "";
    renderedAskAnswer.value = "";
    askStatus.value = "";
...
const downloadCompareResult = async () => {
  if (!compareResult.value) {
    return;
  }

  try {
    const filePath = await downloadComparePDF(compareResult.value);
    compareDownloadSuccess.value = !!filePath;
    setTimeout(() => {
      compareDownloadSuccess.value = false;
    }, 1200);
  } catch {
    compareDownloadSuccess.value = false;
  }
...
disposable(
  PLMainAPI.preferenceService.onChanged(
    ...
    async () => {
      ...
      if (launchMode !== "compare" && launchMode !== "graph") {
        return;
      }
      if (launchMode === "graph") {
        exportMode.value = "Graph";
        askAnswer.value = "";
        renderedAskAnswer.value = "";
        askStatus.value = "";
        compareResult.value = null;
...
<template>
  ...
  >
    ...
    <div
      ref="contentRef"
      :class="
        isCompareMode || isGraphMode
          ? 'flex-none overflow-hidden'
          : 'min-h-0 flex-1 overflow-hidden'
      "
    >
      <div v-if="!isGraphMode" class="w-full px-2 pt-1">
        <div
      ...
      <div
        v-if="isGraphMode"
        class="mx-2 mt-2 h-[520px] overflow-hidden rounded-md bg-neutral-100 px-3 py-2 text-xs leading-5 text-neutral-700 dark:bg-neutral-800 dark:text-neutral-200"
      >
        <PaperGraphView
          embedded
          :external-search-text="searchText"
          :search-submit-tick="graphSearchSubmitTick"
        />
      </div>
        ...
        <div>
          <div class="mb-1 flex items-center justify-between gap-2">
            ...
            <button
              ...
              :disabled="!compareResult.rows.length"
              title="Download PDF"
              @click="downloadCompareResult"
            >
              <BIconCheck v-if="compareDownloadSuccess" class="text-xs" />
    ...
    >
      ...
      <div v-if="isAskMode || isGraphMode" class="flex my-auto space-x-1">
        <button
          v-if="isAskMode"
